...
## 实现后的实测结果（2026-09-14 补充）

切分算法最终改为**两阶段卷名发现**（原设计的「封闭标记集合」已废弃，实测只覆盖 127/198 章）：

1. 用已知标记（`序章|第X章|终章|后记|插图|尾声|番外`）bootstrap 出候选卷名；
2. 剪掉「卷名 + 子标题」形态的碎片；
3. 用卷名把该卷下所有章节标题行扩展出来（自由形式的标题也能覆盖）。

**泛化性抽样（真实 CDN 下载 + 真实切分器）：**

| 书 | 字节 | 章节 | 卷 | 降级 | 偏移失配 |
|---|---|---|---|---|---|
| aid=1011 | 5.9 MB | 274 | 21 | 否 | 0 |
| aid=3020 | 5.2 MB | 168 | 12 | 否 | 0 |
| aid=2580 | 5.1 MB | 295 | 16 | 否 | 0 |
| aid=1（文学少女） | 6.0 MB | 198 | 17 | 否 | 0 |

**4/4 成功切分，0 偏移失配。** 原「未决风险」中担忧的泛化性问题已消除。

**关键修正：CRLF。** 首轮抽样 3 本全部降级为单章，根因是换行符不一致 ——
抽样 3 本均为 CRLF，而《文学少女》为 LF。原实现只按 `\n` 切行，CRLF 文件每行
末尾残留 `\r`，使标题行正则的 `$` 锚点失配。修正为按 `\r?\n` 切分并剥离行尾 `\r`。

**已知边界（可接受）：** 目录中可能残留极少数标题为「插图」但正文非空的条目
（其后的自由形式短篇标题未被识别，正文被并入该条）。不影响导入完整性，仅目录多一项。

**章节正文完整性：** aid=1011 的 274 个切分章节中，20 个为完全空的「插图」，
已被 `hasNoReadableText` 过滤（`volumes()` 产出 254 章，空正文 0），
因此不会触发 `ContentImportService` 的空正文检查中断整本导入。
