...
fun NavGraphBuilder.feedNavGraph(navController: NavHostController) {
    composable(FEED_AUTHOR_ROUTE, listOf(navArgument(FEED_USER_ID_ARG) { type = NavType.StringType })) { entry ->
        ...
        PublicAuthorScreen(userId, onOpenFollowing = { id -> navController.navigate("feed_author_relationship/$id/following") }, onOpenFollowers = { id -> navController.navigate("feed_author_relationship/$id/followers") }) { navController.popBackStack() }
    }
    composable("feed_author_relationship/{userId}/{type}", listOf(navArgument("userId") { type = NavType.StringType }, navArgument("type") { type = NavType.StringType })) { entry ->
        PublicRelationshipScreen(entry.arguments?.getString("userId").orEmpty(), entry.arguments?.getString("type") == "followers")
    }
