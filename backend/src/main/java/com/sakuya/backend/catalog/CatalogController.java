...
import com.sakuya.backend.content.ContentCatalogService;
...
public class CatalogController {
 private final BookRepository books; private final UserLibraryRepository library; private final Wenku8CatalogCacheService wenku8Cache; private final ContentCatalogService contentCatalog; public CatalogController(BookRepository books,UserLibraryRepository library,Wenku8CatalogCacheService wenku8Cache,ContentCatalogService contentCatalog){this.books=books;this.library=library;this.wenku8Cache=wenku8Cache;this.contentCatalog=contentCatalog;}
  ...
  */
 private List<Book> loadWenku8(Wenku8CatalogFeed feed){List<Book> unified=contentCatalog.books(feed.name());if(!unified.isEmpty())return unified;List<Book> cached=wenku8Cache.cached(feed);return cached.isEmpty()?localFallback(feed):cached;}
